...
const char *base_dir = "/sys/devices/system/cpu/"; /** Unix system location of cache information */
...
set_ccpus (struct cache *c)
{
  ...
  if (p) {
    if (fscanf(p, "%s", s1) == 0) {
      fprintf(stderr, "\n\ncache_opt.c, function set_ccpus: unable to find the wanted string in the file %s/%s.\n"
              , c -> datdir, "shared_cpu_list");
...
set_size (struct cache *c)
{
  ...
  if (p) {
    if (fscanf(p, "%d", &i) == 0) {
      fprintf(stderr, "\n\ncache_opt.c, function set_size: unable to find the wanted integer in the file %s/%s.\n"
              , c -> datdir, "size");
      printf("program terminating due to the previous error.\n");
      exit(EXIT_FAILURE);
    }
...
set_lvl (struct cache *c)
{
  ...
  if (p) {
    if (fscanf(p, "%d", &i) == 0) {
      fprintf(stderr, "\n\ncache_opt.c, function set_lvl: unable to find the wanted integer in the file %s/%s.\n"
              , c -> datdir, "level");
      printf("program terminating due to the previous error.\n");
      exit(EXIT_FAILURE);
    }
...
set_lsz (struct cache *c)
{
  ...
  if (p) {
    if (fscanf(p, "%d", &i) == 0) {
      fprintf(stderr, "\n\ncache_opt.c, function set_lsz: unable to find the wanted integer in the file %s/%s.\n"
              , c -> datdir, "coherency_line_size");
      printf("program terminating due to the previous error.\n");
      exit(EXIT_FAILURE);
    }
    fclose(p);
...
set_assoc (struct cache *c)
{
  ...
  if (p) {
    if (fscanf(p, "%d", &i) == 0) {
      fprintf(stderr, "\n\ncache_opt.c, function set_assoc: unable to find the wanted integer in the file %s/%s.\n"
              , c -> datdir, "ways_of_associativity");
      printf("program terminating due to the previous error.\n");
      exit(EXIT_FAILURE);
    }
    fclose(p);
...
set_type (struct cache *c)
{
  ...
  char type;
  if (p) {
    if (fscanf(p, "%c", &type) == 0) {
      fprintf(stderr, "\n\ncache_opt.c, function set_type: unable to find the wanted character in the file %s/%s.\n"
              , c -> datdir, "type");
      printf("program terminating due to the previous error.\n");
      exit(EXIT_FAILURE);
    }
...
cache2file (char *log_fn)
{
  ...
  if((log_fp = fopen(log_fn, "w")) == NULL) {
    fprintf(stderr, "\n\ncache_opt.c, function cache2file: unable to open the file %s.\n"
            , log_fn);
    printf("program terminating due to the previous error.\n\n");
    exit(1);
  }
  ...
  fprintf(log_fp, "\n====================== Start ======================\n" );
  fprintf(log_fp, "============ Cache topology information ===========\n\n");

  fprintf(log_fp, "Cache topology data generated by the cache2file function (see cache_opt.c or program documentation for more information).\n" );
  while(last_cl != NULL){

    fprintf(log_fp, "Cache lvl %d\n", last_cl -> lvl );
    c = last_cl -> root_cache;

    while(c != NULL){
      if (c -> type != 'I' ) {
        fprintf(log_fp, "\tlvl = %d\n", c -> lvl);
        fprintf(log_fp, "\tl_sz = %d\n", c -> l_sz);
        fprintf(log_fp, "\ttot_sz = %d\n", c -> tot_sz);
        fprintf(log_fp, "\ttype = %c\n", c -> type);
        fprintf(log_fp, "\tassoc = %d\n", c -> assoc);
        fprintf(log_fp, "\tnl = %d\n", c -> nl);
        fprintf(log_fp, "\tn_ccpu = %d\n", c -> n_ccpu);
        fprintf(log_fp, "\tn_fpl = %d\n", c -> n_fpl);
        fprintf(log_fp, "\tn_dpl = %d\n", c -> n_dpl);
        for (j = 0; j < c -> n_ccpu; j++) {
          fprintf(log_fp, "\t  ccpus[%d] = %d\n", j, c -> ccpus[j]);
        }
        fprintf(log_fp, "\n" );
      }
  ...
  if(fclose(log_fp) != 0) {
    fprintf(stderr, "\n\ncache_opt.c, function cache2file: unable to close the file %s.\n"
            , log_fn);
    printf("program terminating due to the previous error.\n\n");
    exit(1);
  }
